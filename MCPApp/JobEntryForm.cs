...
namespace MCPApp
{
    public partial class JobEntryForm : Form
    {
        ...
        private string JobsDGVToDB()
        {
            ...
            try
            {
                for (int i = 0; i < jobDGV.Rows.Count; i++)
                {
                    ...
                    phaseInvValue = jobDGV.Rows[i].Cells[17].Value == null || jobDGV.Rows[i].Cells[17].Value == "" ? 0 : Convert.ToDecimal(jobDGV.Rows[i].Cells[17].Value.ToString());
                    jobMgnValue = jobDGV.Rows[i].Cells[18].Value == null || jobDGV.Rows[i].Cells[18].Value == "" ? 0 : Convert.ToDecimal(jobDGV.Rows[i].Cells[18].Value.ToString());
        ...
        private void jobDGV_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            ...
            if (jobDGV.CurrentCell.ColumnIndex == 9 || jobDGV.CurrentCell.ColumnIndex == 10 || jobDGV.CurrentCell.ColumnIndex == 11 || jobDGV.CurrentCell.ColumnIndex == 16) //Desired Column
            {
