...
namespace MCPApp
{
    public partial class NewDesignBoardForm : Form
    {
        ...
        private void BuildTabs()
        {
            ...
            for (int i = 0; i < wcNumWeeks; i++)
            {
               ...
               // weeksTabControl.TabPages[i].AutoScrollOffset = 
            ...
            //MessageBox.Show($"DGV Count = {dgvCount} and Vertical Scroll Bar Count = {verticalScrollBarCount}");
        }
        ...
        private void NewDesignBoardForm_Load(object sender, EventArgs e)
        {
            DateTime startDate = new DateTime(2024, 4, 1);
            DateTime lastDate = new DateTime(2024, 4, 28);
            wbDatesDT = mcData.DatesDT(startDate, lastDate);
            TimeSpan ts = lastDate - startDate;
            int dateDiff = ts.Days;
            decimal numWeeks = dateDiff / 7m;
            wcNumWeeks = (int)Decimal.Round(numWeeks, 1) + 1;
            BuildTabs();
        }
