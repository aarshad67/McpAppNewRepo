...
namespace MCPApp
{
    public class Crypto
    {
        ...
        private byte[] EncryptDecrypt(byte[] inputBytes, bool Encrpyt)
        {
            ...
            {
                string msg = String.Format("EncryptDecrypt() Error : {0}", ex.Message.ToString());
                string audit = mcData.CreateErrorAudit("Crypto.cs", "EncryptDecrypt(byte[] inputBytes, bool Encrpyt)", msg);
                logger.LogLine(msg);
                return null;
            }
        }
